...
class sql:
    ...
    def create_table(self,mytable):
        create_table_sql = "CREATE TABLE IF NOT EXISTS  `"+mytable+ "` (name varchar(20)) CHARACTER SET utf8"  
        try:
    ...
    def insert_sql(self,data):
        ...
        tablenum=0
        try:
            select_table_sql="SELECT * FROM `"+data[0]+"`"
            ...
            pass
        if tablenum<1:
            insert_sql = "INSERT INTO `"+data[0]+"` values(%s)"
            val=[data[1]]
            try:
...
pp=sql()
i=0
a=pp.create_database('maplist')
for dat in data:
    b=pp.create_table(dat[0])
    if b==1:
        pp.insert_sql(dat)   
        i=i+1
        print(i)
