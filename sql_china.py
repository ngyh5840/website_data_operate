...
class sql:
    ...
    def create_table(self,mytable):
        create_table_sql = "CREATE TABLE IF NOT EXISTS  `"+mytable+ "` (province varchar(10),province_en varchar(10), \
            area varchar(10), city_en varchar(10),area_en varchar(10),area_num int(5)) CHARACTER SET utf8"  
        try:
    ...
    def insert_sql(self,data):
        tablenum=0
        try:
            select_table_sql="SELECT * FROM `"+data.city+"`"
            ...
            pass
        if tablenum<1:
            insert_sql = "INSERT INTO `"+data.city+"` values(%s,%s,%s,%s,%s,%s)"
            val=[data.province,data.province_en,data.area,data.city_en,data.area_en,data.area_num]
            try:
...
p=inputdata()
data=p.get_china_data()
pp=sql()
i=0
a=pp.create_database('chinadata')
for dat in data:
